...
function generate_import_statement(p::FromPackageController, ex::Expr, starting_module::Module; pop_first::Bool=true, exclude_usings::Bool=false)
    ...
    if catchall
        ...
        return make_using_explicit(import_module, id; is_import = ex.head === :import)
    end
end
...
function process_input_statement(p::FromPackageController, out::ImportStatementData; pop_first::Bool = false, allow_manifest = false, is_import::Bool = true)
    ...
    ex = make_using_explicit(target_module, out; is_import)
    return ex
end
...
function make_using_explicit(target_module::Module, id::ImportStatementData; is_import::Bool)
    (; modname_path, imported_names) = id
    ex = if isempty(imported_names)
        ...
        reconstruct_import_statement(:import, ImportStatementData(modname_path, nms))
    else
        # We have an explicit list of names, so we simply modify the using to import
        reconstruct_import_statement(:import, id)
    end
    return ex
end
