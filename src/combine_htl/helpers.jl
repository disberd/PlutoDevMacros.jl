...
shouldskip(x::PlutoScript) = shouldskip(x.body) && shouldskip(x.invalidation) && !hasreturn(x)
shouldskip(x::NormalScript) = shouldskip(x.body) && !hasreturn(x)
...
# Dual Script/Node
shouldskip(n::Node) = shouldskip(n, displaylocation(n))
